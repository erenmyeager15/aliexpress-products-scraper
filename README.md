...
Collect structured public AliExpress product catalog and pricing data for market research and price monitoring. Export clean records with source, search query, position, product ID, title, brand, price, MRP, discount, currency, pack size, category, rating, stock status, product URL, image URL, and timestamp. Export to JSON, CSV, Excel, or HTML, or pull results through the Apify API. No AliExpress login or API key is required.

For the first run, start small: one search query, `maxResults` between 5 and 10, `maxPagesPerQuery` set to 1, and the recommended residential proxy enabled.
...
- Aggregate rating, rating count when exposed, and stock status when exposed
...
| `maxResults` | integer | no | `10` | Maximum unique products, up to 500. Use 5-10 for tests |
...
This Actor uses Apify Pay Per Event + platform usage pricing. You pay for clean product records saved to the dataset, and you also pay the Apify platform usage generated by the run, such as compute and residential proxy traffic.
...
- Use `maxResults` between 5 and 10 for the first test.
...
2. For a first run, use 5-10 products and 1 page per query.
